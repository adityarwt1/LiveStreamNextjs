...
app.prepare().then(() => {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("join-stream", async (data) => {
      ...
      console.log(`[Socket] Viewer ${viewerId} joining stream ${streamId}`);
      ...
      if (stream) {
        ...
        // Notify streamer about new viewer and request offer
    ...
    socket.on("answer", (data) => {
      console.log(`[Socket] Answer from ${socket.id} to streamer`);
    ...
    socket.on("disconnect", async () => {
      ...
      for (const [streamId, stream] of activeStreams.entries()) {
        if (stream.streamerId === socket.id) {
          console.log(
            `[Socket] Streamer disconnected, ending stream: ${streamId}`
          );
          io.to(streamId).emit("stream-ended");
          activeStreams.delete(streamId);
          streamViewers.delete(streamId);
        ...
        if (stream.viewers.has(socket.id)) {
          ...
          try {
            ...
            console.error(
              "[Socket] Error updating viewer count on disconnect:",
              error
            );
          }
        }
